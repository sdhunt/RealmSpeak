...
/**
 * A component that represents an attribute of some sort.
 */
// This class is not used in the codebase.
@Deprecated
public class AttributeBar extends JComponent {
    ...
    private final Color realColor = new Color(0, 0, 255);
    private final Color projColor = new Color(0, 0, 255, 50);
    ...
    /**
     * Creates an attribute bar with the specified maximum value.
     *
     * @param max maximum value for this attribute
     */
    ...
    /**
     * Generates a key for this attribute bar, representing its state.
     *
     * @return a key string
     */
    ...
    /**
     * Parses a key string and sets the state of this attribute bar
     * accordingly.
     *
    ...
    public void readKey(String key) {
        ...
        maxVal = Integer.valueOf(tokenz.nextToken());
        realVal = Integer.valueOf(tokenz.nextToken());
        projVal = Integer.valueOf(tokenz.nextToken());
        goalVal = Integer.valueOf(tokenz.nextToken());
        normalize();
        repaint();
    }
    ...
    /**
     * Returns the name associated with this bar.
     *
     * @return the name
     */
    @Override
    ...
    private void setRealValue(int val) {
        realVal = val;
        normalize();
        repaint();
    }
    ...
    private void setProjectedValue(int val) {
        projVal = val;
        normalize();
        repaint();
    }
    ...
    public void paintComponent(Graphics g1) {
        ...
        Rectangle realValRect =
                new Rectangle(DIAMOND_REGION, 0,
                              maxTextBox.width - 1, size.height - 1);
        Rectangle projValRect =
                new Rectangle(size.width - maxTextBox.width, 0,
                              maxTextBox.width - 1, size.height - 1);

        Rectangle barRect =
                new Rectangle(realValRect.x + realValRect.width, 0,
                              size.width - realValRect.width - projValRect.width - DIAMOND_REGION - 1, size.height - 1);
        ...
        if (goalVal > 0) {
            ...
            drawCenteredString(g, goalValRect.x, goalValRect.y, goalValRect.width, goalValRect.height, "" + goalVal);
        }

        drawCenteredString(g, realValRect.x, realValRect.y, realValRect.width, realValRect.height, getStringForValue(realVal));
        if (projVal > realVal) {
            drawCenteredString(g, projValRect.x, projValRect.y, projValRect.width, projValRect.height, getStringForValue(projVal));
        }
